...
<div class="bg-white shadow-2xl rounded-3xl p-10 w-1/2">
    ...
    <form action="/edit" method="POST">
        ...
        <select name="role" class="w-full px-4 py-3 bg-gray-100 rounded-xl mb-4">
            <option value="VOLUNTEER" ${user.role == 'VOLUNTEER' ? 'selected' : ''}>Volunteer</option>
            <option value="ORGANIZER" ${user.role == 'ORGANIZER' ? 'selected' : ''}>Organizer</option>
        </select>

        <label class="block mb-3 font-semibold">Skills</label>

        <c:forEach items="${allSkills}" var="skill">
            <div class="flex items-center mb-2">
                <input type="checkbox" name="skills" value="${skill.id}"
                    ${user.skills.contains(skill) ? 'checked' : ''}/>
